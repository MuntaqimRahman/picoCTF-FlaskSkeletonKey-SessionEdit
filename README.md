...
After obtaining the sessionID (using BurpSuite to intercept the request) it was decoded using the secret key. The secret key was found on the primary upload page using SSTI () Experimenting a little bit, one finds that the user-id is consecutive. It would then follow that the first user is the admin. So I just changed the user-id to 1 so the webpage would think I was the admin, and re-encoded the information with the secret-key.
